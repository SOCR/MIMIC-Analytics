...
project = 'socr-mdp-mimic-analytics'
```
...
# ESCAPE Scoring System

The ESCAPE ([O’connor 2010](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3835158/)) scoring system aims to predict 6-month mortality rates among patients with heart failure risk. In order to apply the ESCAPE scoring system on the MIMIC-III dataset, we retrieved the cohort patients’ age, blood urea nitrogen level, sodium level, diuretic dose, brain natriuretic peptide level at discharge, along with whether they received CPR/mechanical ventilation and whether they received beta-blockers at discharge.

## Data extraction

The `itemid`s of a category are extracted from the `d_items` and and `d_labitems` tables by searching searching for a keyword.
...
The CPR codes 9960, 9390, 967, 9670, 9671, and 9672 were used to find the patients in the cohort who received CPR or mechanical ventilation.
...
## Join all the separate data columns together
The N/A values in the CPR column is imputed to `FALSE`, while the DIURETIC and BETABLOCKERS columns are imputed to 0. The BNP and BUN columns are imputed to the mean of the present data in the columns.

```{r}
joined = left_join(cohort, bnp) %>% left_join(diuretic) %>% left_join(blocker) %>% left_join(cpr) %>% left_join(data.bun) %>% left_join(data.sodium)
...
joined$BUN[is.na(joined$BUN)] = mean(data.bun$BUN, na.rm = TRUE)
joined$Sodium[is.na(joined$Sodium)] = mean(data.sodium$Sodium, na.rm = TRUE)
```

## ESCAPE score calculation
...
Below is a histogram of the computed ESCAPE scores
